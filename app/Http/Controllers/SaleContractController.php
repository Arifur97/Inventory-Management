...
class SaleContractController extends Controller
{
    public function index(Request $request)
    {
        ...
        if($role->hasPermissionTo('purchases-index')) {
            ...
            return view('sale_contract.index', compact( 'lims_account_list', 'lims_warehouse_list', 'all_permission', 'lims_supplier_list', 'lims_supplier_quotation_all', 'warehouse_id', 'starting_date', 'ending_date'));
        }
        else
    ...
    public function create()
    {
        ...
        if($role->hasPermissionTo('quotes-add')){
            ...
            return view('sale_contract.create', compact('lims_customer_list', 'lims_warehouse_list', 'lims_tax_list', 'lims_product_list_without_variant', 'lims_product_list_with_variant'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($multiFile) {
            ...
            foreach ($multiFile as $docfile) {
                ...
                $docfile->move(public_path('document/sale-contract'), $documentName);

                $imageDb[] = $documentName;
            }
            $imageDbUrl = implode(",", $imageDb);

            $data['document'] = 'document/sale-contract'. $imageDbUrl;
        }
        //return dd($data);
        SaleContract::create($data);

        $lims_purchase_data = SaleContract::latest()->first();
        ...
        foreach ($product_id as $i => $id) {
            ...
            $product_purchase['sale_contract_id'] = $lims_purchase_data->id ;
            $product_purchase['product_id'] = $id;
            $product_purchase['qty'] = $qty[$i];
            $product_purchase['recieved'] = $recieved[$i];
            $product_purchase['sale_unit_id'] = $lims_purchase_unit_data->id;
            ...
            ProducSaleContract::create($product_purchase);
        }

        return redirect('sale_contract')->with('message', 'Sale Contract created successfully');
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        foreach ($lims_product_purchase_data as $product_purchase_data) {
            ...
            $lims_purchase_unit_data = Unit::find($product_purchase_data->sale_unit_id);
