...
class RoleController extends Controller
{
    ...
    public function shortcutsPermission($id) {
        $lims_role_data = Roles::findorfail($id);

        $lims_role_data['shortcut'] = json_decode($lims_role_data->shortcut_form_id);

        $getPermissions = Role::findByName($lims_role_data->name)->permissions;
        $permissionFormIds = $getPermissions->pluck('form_id')
            ->filter(fn ($value, $key) => $value != null)->unique();

        $lims_shortcut_all = [];
        foreach($permissionFormIds as $value) {
            $lims_shortcut_all[] = FormName::findorfail($value);
        }

        $permissions = $getPermissions->filter(fn ($value, $key) => $value->form_id != null)->unique();

        $permisisons_str ='';
        foreach($permissions as $permission) {
            if(str_contains($permission->name, 'index')) {
                $permisisons_str .= $permission->name . ', ';
            }
        }

        return view('role.create_shortcuts', compact('lims_role_data', 'permisisons_str', 'lims_shortcut_all'));
    }

    public function updateShortcutsPermission(Request $request, $id) {
        $lims_role_data = Roles::findorfail($id);
        $lims_role_data->shortcut_form_id = json_encode($request->shortcut);
        $lims_role_data->update();

        return redirect()->route('role.index')->with('message', 'Successfully Done');
    }
