...
class TransferController extends Controller
{
    public function index()
    {
        ...
        if($role->hasPermissionTo('transfers-index')){
            ...
            if(Auth::user()->role_id > 2 && config('staff_access') == 'own')
                $lims_transfer_all = Transfer::with('fromWarehouse', 'toWarehouse', 'user', 'documents')->orderBy('id', 'desc')->where('user_id', Auth::id())->get();
            else
                $lims_transfer_all = Transfer::with('fromWarehouse', 'toWarehouse', 'user', 'documents')->orderBy('id', 'desc')->get();
    ...
    public function create()
    {
        ...
        if($role->hasPermissionTo('transfers-add')){
            ...
            return view('transfer.create', compact('lims_warehouse_list', 'lims_product_list'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function limsProductSearch(Request $request)
    {
        ...
        $product[] = url('/images/product/' . $lims_product_data->image);
        return $product;
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($document) {
            ...
            $document->move('/document/transfer', $documentName);
        ...
        foreach ($product_id as $i => $id) {
            ...
            if($data['status'] != 2){
                if (($lims_purchase_unit_data->operator ?? '') == '*')
                    ...
                    $quantity = $qty[$i] / ($lims_purchase_unit_data->operation_value??1);
            }
            else
                $quantity = 0;
            //get product data
    ...
    public function edit($id)
    {
        ...
        if($role->hasPermissionTo('transfers-edit')){
            $lims_warehouse_list = Warehouse::where('is_active',true)->get();
            $lims_transfer_data = Transfer::with('documents')->find($id);
            $lims_product_transfer_data = ProductTransfer::where('transfer_id', $id)->get();
    ...
    public function storeMultifile(Request $request) {
        ...
        try {
            ...
            $doc = Attachments::store($documents, 'App\Transfer');
    ...
    public function updateMultifile(Request $request) {
        ...
        try {
            ...
            $doc = Attachments::docUpdate($documents, 'App\Transfer', $oldDocs, $id);
            return response()->json([
                'success' => true,
                'id' => $id ?? $doc->id,
            ]);
