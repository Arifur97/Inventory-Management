...
class SettingController extends Controller
{
    ...
    public function generalSetting()
    {
        ...
        return view('setting.general_setting', compact('lims_general_setting_data', 'lims_account_list', 'zones_array', 'lims_currency_list', 'form_name_data'));
    }
    ...
    public function generalSettingStore(Request $request)
    {
        ...
        $this->validate($request, [
            'site_icon' => 'image|mimes:jpg,jpeg,png,gif|max:100000',
        ]);

        $data = $request->except('site_icon');
        //return $data;
        ...
        $icon = $request->site_icon;
        if ($icon) {
            $ext = pathinfo($icon->getClientOriginalName(), PATHINFO_EXTENSION);
            $iconName = date("Ymdhis") . '.' . $ext;
            $icon->move('/icon', $iconName);
            $general_setting->site_icon = $iconName;
        }
