...
class PurchaseController extends Controller
{
    public function index(Request $request)
    {
        ...
        if($role->hasPermissionTo('purchases-index')) {
            ...
            if($request->input('purchase_status_id'))
            ...
            return view('purchase.index', compact( 'lims_account_list', 'lims_warehouse_list', 'lims_supplier_list', 'lims_status_list', 'lims_payment_status_list', 'all_permission', 'lims_pos_setting_data', 'warehouse_id','supplier_id', 'starting_date', 'ending_date', 'reference_no', 'payment_status_id', 'purchase_status_id'));
        }
        else
    ...
    public function purchaseData(Request $request)
    {
        ...
        elseif($supplier_id != 0)
            $totalData = Purchase::where('supplier_id', $supplier_id)->whereDate('created_at', '>=' ,$request->input('starting_date'))->whereDate('created_at', '<=' ,$request->input('ending_date'))->count();
        else
        ...
        if(empty($request->input('search.value'))) {
            if(Auth::user()->role_id > 2 && config('staff_access') == 'own') {
                ...
                $purchases = Purchase::with('supplier', 'warehouse')->offset($start)
                            ...
                            ->orderBy($order, $dir);
                if($warehouse_id) {
                    $purchases = $purchases->where('warehouse_id', $warehouse_id);
                }
                if($supplier_id) {
                    $purchases = $purchases->where('supplier_id', $supplier_id);
                }
                if($reference_no) {
                    $purchases = $purchases->where('reference_no', 'LIKE', "%{$reference_no}%");
                }
                if($payment_status_id) {
                    $purchases = $purchases->where('payment_status', $payment_status_id);
                }
                if($purchase_status_id) {
                    $purchases = $purchases->where('status', $purchase_status_id);
                }
        ...
        if(!empty($purchases))
        {
            foreach ($purchases as $key=>$purchase)
            {
                ...
                elseif($purchase->status == 2){
                    $nestedData['purchase_status'] = '<div class="badge badge-primary">'.trans('file.Partial').'</div>';
                ...
                elseif($purchase->status == 3){
                    ...
                    $nestedData['purchase_status'] = '<div class="badge badge-info">'.trans('file.Ordered').'</div>';
                ...
                elseif($purchase->payment_status == 2){
                    $nestedData['payment_status'] = '<div class="badge badge-primary">'.trans('file.Partial').'</div>';
                }
                else{
    ...
    public function store(Request $request)
    {
        ...
        if ($document) {
            ...
            $document->move('/document/purchase', $documentName);
