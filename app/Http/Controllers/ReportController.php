...
class ReportController extends Controller
{
    ...
    public function profitLossLocation(Request $request)
    {
        $start_date = $request['start_date'];
        $end_date = $request['end_date'];
        $query1 = array(
        ...
        foreach ($lims_warehouse_all as $warehouse) {
            $warehouse_name[] = $warehouse->name;
            $warehouse_sale[] = Sale::where('warehouse_id', $warehouse->id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->selectRaw(implode(',', $query1))->get();
            $warehouse_purchase[] = Purchase::where('warehouse_id', $warehouse->id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->selectRaw(implode(',', $query1))->get();
            $warehouse_return[] = Returns::where('warehouse_id', $warehouse->id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->selectRaw(implode(',', $query2))->get();
            $warehouse_purchase_return[] = ReturnPurchase::where('warehouse_id', $warehouse->id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->selectRaw(implode(',', $query2))->get();
            $warehouse_expense[] = Expense::where('warehouse_id', $warehouse->id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->sum('amount');
        }

        return view('report.profit_loss_location', compact('purchase', 'total_purchase', 'sale', 'total_sale', 'return', 'purchase_return', 'total_return', 'total_purchase_return', 'expense', 'payroll', 'total_expense', 'total_payroll', 'payment_recieved', 'payment_recieved_number', 'cash_payment_sale', 'cheque_payment_sale', 'credit_card_payment_sale', 'gift_card_payment_sale', 'paypal_payment_sale', 'deposit_payment_sale', 'payment_sent', 'payment_sent_number', 'cash_payment_purchase', 'cheque_payment_purchase', 'credit_card_payment_purchase', 'warehouse_name', 'warehouse_sale', 'warehouse_purchase', 'warehouse_return', 'warehouse_purchase_return', 'warehouse_expense', 'start_date', 'end_date'));
    }
    ...
    public function allCustomerIndex(Request $request)
    {
        ...
        $returnsum  = 0;
        $paymnetsum = 0;
        foreach ($lims_customer_all as $key => $value) {
            $sale        = Sale::where('customer_id', '=',$value['id'])->get('grand_total');
            $return     = Returns::where('customer_id', '=',$value['id'])->get('grand_total');
            $payment    = Sale::where('customer_id', '=',$value['id'])->get('paid_amount');
            ...
            foreach($return as $v) {
                $returnsum += $v['grand_total'];
            }

            foreach($payment as $v) {
                $paymnetsum += $v['paid_amount'];
            }

            $chunk[$key]['customer'] = $value;
            $chunk[$key]["sale"] = $sum;
            $sum = 0;
            $chunk[$key]["return"] = $returnsum;
            $returnsum = 0;

            $chunk[$key]["paid"] = $paymnetsum;
            $paymnetsum = 0;
        ...
        return view('report.all_customer_due', compact('lims_customer_all', 'chunk'));
    }

    public function allSupplierIndex(Request $request)
    {
        ...
        $returnsum  = 0;
        $paymnetsum = 0;
        foreach ($lims_supplier_all as $key => $value) {
            $pur        = Purchase::where('supplier_id', '=',$value['id'])->get('grand_total');
            $return     = ReturnPurchase::where('supplier_id', '=',$value['id'])->get('grand_total');
            $payment    = Purchase::where('supplier_id', '=',$value['id'])->get('paid_amount');
            ...
            $chunk[$key]['supplier'] = $value;
            $chunk[$key]["purchase"] = $sum;
            $sum = 0;
            $chunk[$key]["return"] = $returnsum;
            $returnsum = 0;
        ...
        return view('report.all_supplier_due', compact('lims_supplier_all', 'chunk'));
    }

    public function productCostReport()
    {

        $lims_product_all = Product::join('units', 'products.unit_id', '=', 'units.id')
        ->join('categories', 'products.category_id', '=', 'categories.id')
        ->join('product_purchases', 'products.id', '=', 'product_purchases.product_id')

        ->select('categories.name as category_name', 'products.id', 'products.image', 'products.name', 'products.code', 'products.qty', 'products.category_id', 'units.unit_name', 'products.cost', 'product_purchases.net_unit_cost', 'product_purchases.discount', 'product_purchases.updated_at')
        ->orderBy('products.name', 'asc')
        ->get();

        foreach($lims_product_all as $key => $value) {
            $max_product_cost = 0;
            $sum_product_cost = 0;
            $index_product_cost = 0;
            $updated_at_product_cost = $lims_product_all[$key]['updated_at'];
            foreach($lims_product_all as $innerkey => $innervalue) {
                if($lims_product_all[$key]['id'] == $lims_product_all[$innerkey]['id']) {
                    $index_product_cost++;
                    // highest cost
                    $minus_discount_from_product = $lims_product_all[$innerkey]['net_unit_cost'] - $lims_product_all[$innerkey]['discount'];
                    $max_product_cost = $max_product_cost > $minus_discount_from_product ? $max_product_cost : $minus_discount_from_product;
                    $lims_product_all[$key]['highest_cost'] = $max_product_cost;

                    // avg cost
                    $sum_product_cost += $lims_product_all[$innerkey]['net_unit_cost'] - $lims_product_all[$innerkey]['discount'];

                    // latest cost
                    $updated_at_product_cost = $updated_at_product_cost->gt($lims_product_all[$innerkey]['updated_at']) ? $updated_at_product_cost : $lims_product_all[$innerkey]['updated_at'];
                    $bool_updated_at_product_cost = $updated_at_product_cost->gt($lims_product_all[$innerkey]['updated_at']);
                    if($bool_updated_at_product_cost) {
                        continue;
                    } else {
                        $lims_product_all[$key]['latest_cost'] = $lims_product_all[$innerkey]['net_unit_cost'] - $lims_product_all[$innerkey]['discount'];
                    }

                }
            }
            // avg cost
            if($index_product_cost <= 0) {
                $index_product_cost = 1;
            }
            $lims_product_all[$key]['avg_cost'] = $sum_product_cost / $index_product_cost;
        }

        $lims_product_all_array = json_decode($lims_product_all);
        $temp_product_all = array_unique(array_column($lims_product_all_array, 'id'));
        $product_all_data = array_intersect_key($lims_product_all_array, $temp_product_all);



        $warehouse_list = Product::join('product_warehouse', 'products.id', '=', 'product_warehouse.product_id')
        ->join('warehouses', 'product_warehouse.warehouse_id', '=', 'warehouses.id')
        ->get(['product_warehouse.product_id', 'warehouses.name', 'product_warehouse.qty']);





        // return response()->json($product_all_data);

        $lims_warehouse_list = Warehouse::where('is_active', true)->get();


        return view('report.product_cost_report',compact('product_all_data', 'lims_warehouse_list', 'warehouse_list'));
    ...
    public function stockavailability(Request $request)
    {
        ...
        $lims_product_all = Product::join('units', 'products.unit_id', '=', 'units.id')
        ->get(['products.id', 'products.image', 'products.name', 'products.code', 'products.qty', 'products.category_id', 'units.unit_name']);
        ...
        return view('report.all_stock_ability_due',compact('lims_product_all', 'lims_warehouse_list', 'warehouse_list'));
