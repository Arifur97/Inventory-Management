...
class ProductController extends Controller
{
    public function index(Request $request)
    {
        ...
        if($role->hasPermissionTo('products-index')){
            ...
            $lims_type_list     = Unit::where('is_active', true)->get();

            return view('product.index', compact('all_permission', 'lims_category_list', 'category_id', 'brand_id', 'lims_brand_list', 'brand_id', 'lims_unit_list', 'unit_id', 'type_id', 'lims_type_list'));
        }
        else
    ...
    public function productData(Request $request)
    {
        ...
        elseif($brand_id != 0)
            $totalData = Product::where('brand_id', $brand_id)->count();
        elseif($unit_id != 0)
            $totalData = Product::where('unit_id', $unit_id)->count();
        else
        ...
        if(empty($request->input('search.value'))){
            if(Auth::user()->role_id > 2 && config('staff_access') == 'own') {
                ...
                if($unit_id) {
                    $products = $products->where('unit_id', $unit_id);
                }
        ...
        else
        {
            ...
            $products =  Product::select('products.*')
                        ...
                        // ->join('producttypes', 'products.type', '=', 'producttypes.name')
                        ...
                        // ->orWhere([
                        //     ['producttypes.name', 'LIKE', "%{$search}%"],
                        //     ['producttypes.is_active', true],
                        //     ['products.is_active', true]
                        // ])
        ...
        if(!empty($products))
        {
            foreach ($products as $key=>$product)
            {
                ...
                $nestedData['image'] = '<img src="'.url('/images/product', $product_image).'" height="80" width="80" class="product_image">';
                ...
                $nestedData['other_code'] = $product->other_code;
                ...
                if($product->type)
                    ...
                    $nestedData['type'] = "N/A";
                // $nestedData['p_type'] = $product->type;
                // $nestedData['type'] = $product->producttype->name;
                // if($product->type)
                //     $nestedData['p_type'] = $product->name;
                // else
                //     $nestedData['p_type'] = 'N/A';

                // if($product->type == 1)
                //     $nestedData['type'] = '<div class="badge badge-success">'.trans('file.Stockable').'</div>';
                // elseif($product->type == 2)
                //     $nestedData['type'] = '<div class="badge badge-bule">'.trans('file.Non-stockable').'</div>';
                // elseif($product->type == 3)
                //     $nestedData['type'] = '<div class="badge badge-primary">'.trans('file.Combo').'</div>';
                // elseif($product->type == 4)
                //     $nestedData['type'] = '<div class="badge badge-danger">'.trans('file.Temporary').'</div>';
                // else
                //     $nestedData['type'] = '<div class="badge badge-success">'.trans('file.Stockable').'</div>';

                if($product->is_merchandise == 0)
                    $nestedData['is_merchandise'] = '<div class="badge badge-danger">'.trans('file.No').'</div>';
                else
                    $nestedData['is_merchandise'] = '<div class="badge badge-success">'.trans('file.Yes').'</div>';

                if($product->is_active == 0)
                    $nestedData['is_active'] = '<div class="badge badge-danger">'.trans('file.No').'</div>';
                else
                    $nestedData['is_active'] = '<div class="badge badge-success">'.trans('file.Yes').'</div>';
    ...
    public function create()
    {
        ...
        if ($role->hasPermissionTo('products-add')){
            ...
            $lims_brand_list = Brand::where('is_active', true)->get();
            $lims_producttype_list = ProductType::all();
            $lims_yesno_list = YesNo::all();
            $lims_category_list = Category::where('is_active', true)->get();
            $lims_unit_list = Unit::where('is_active', true)->get();
            $lims_tax_list = Tax::where('is_active', true)->get();
            $lims_warehouse_list = Warehouse::where('is_active', true)->get();
            return view('product.create',compact('lims_product_list', 'lims_brand_list', 'lims_category_list', 'lims_unit_list', 'lims_tax_list', 'lims_producttype_list', 'lims_yesno_list', 'lims_warehouse_list'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function edit($id)
    {
        ...
        if ($role->hasPermissionTo('products-edit')) {
            ...
            return view('product.edit',compact('lims_product_list', 'lims_brand_list', 'lims_category_list', 'lims_unit_list', 'lims_tax_list', 'lims_product_data','lims_producttype_list', 'lims_yesno_list', 'lims_product_variant_data', 'lims_warehouse_list'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function importProduct(Request $request)
    {
        ...
        while($columns=fgetcsv($file))
        {
           ...
           // Code is unique (and not name) Fields as per csv excel column names as below
           ...
           $product->is_merchandise = $data['merchandise'];
