...
Route::group(['middleware' => ['auth', 'active']], function() {
	...
	Route::get('dashboard/dashboard-view', 'HomeController@index');
    ...
    Route::get('/user/default-company/{id}/change', 'UserGeneralSettingController@updateOrCreateCompanyId')->name('user.default-company.change');

	Route::resource('human_resource', 'HomeController');
	Route::get('human_resource', 'HomeController@humanResource')->name('human_resource.index');

	// weekly graph api
	Route::get('/dashboard/v1/weekly-graph', 'HomeController@weeklyGraphApi');

	Route::get('language_switch/{locale}', 'LanguageController@switchLanguage');

	Route::get('role/shortcuts_permission/{id}', 'RoleController@shortcutsPermission')->name('role.permission.shortcuts');
	...
	Route::post('systemcolor/deletebyselection', 'SystemColorController@deleteBySelection');
	Route::get('systemcolor/lims_warehouse_search', 'SystemColorController@limsWarehouseSearch')->name('systemcolor.search');
	Route::resource('systemcolor', 'SystemColorController');

	Route::get('/workflow/stage-data/{id}', 'WorkflowSetupController@getWorkflowStageData');
	Route::resource('workflow', 'WorkflowSetupController');
	...
	Route::post('products/getSelectedProducts','ShelfLocationController@getSelectedProducts');
	Route::get('products/shelf_location/create','ShelfLocationController@create')->name('product.shelf_location.create');
	Route::post('products/shelf_location/store','ShelfLocationController@store')->name('products.shelf_store');
	Route::get('products/shelf_location/{id}/destroy','ShelfLocationController@destroy')->name('product.shelf_location.destroy');
	Route::get('products/shelf_location/{id}/edit','ShelfLocationController@edit')->name('product.shelf_location.edit');
	Route::post('products/shelf_location/{id}/update','ShelfLocationController@update')->name('product.shelf_location.update');
	Route::post('products/shelf_location/import','ShelfLocationController@importData')->name('product.shelf_location.import');
	Route::get('products/shelf_location','ShelfLocationController@index')->name('product.shelf_location');
    ...
    //sales route
    Route::post('sales/getSelectedProducts','SaleController@getSelectedProducts');
	...
	// sale contract
	Route::get('sale_contract/lims_sale_search', 'SaleContractController@limsSaleSearch')->name('sale_contract.search');
	Route::get('sale_contract/lims_product_search', 'SaleContractController@limsProductSearch')->name('sale_contract.search');
	Route::resource('sale_contract', 'SaleContractController');

	// Item requirement
	Route::get('item_requirement/lims_sale_search', 'ItemRequirementController@limsSaleSearch')->name('item_requirement.search');
	Route::get('item_requirement/lims_product_search', 'ItemRequirementController@limsProductSearch')->name('item_requirement.search');
	Route::get('item_requirement/view/{id}', 'ItemRequirementController@view')->name('item_requirement.view');
	Route::resource('item_requirement', 'ItemRequirementController');
    Route::post('item_requirement/getSelectedProducts', 'ItemRequirementController@getSelectedProducts');
    // Route::post('item_requirement/product-data', 'ItemRequirementController@productData');

    //taks
    Route::post('/task/multifile/store', 'TaskController@storeMultifile')->name('task.multifile.store');
	Route::post('/task/multifile/update', 'TaskController@updateMultifile')->name('task.multifile.update');
    Route::resource('task', 'TaskController');

    //support ticker
    Route::post('/support_ticket/multifile/store', 'SupportTicketController@storeMultifile')->name('support_ticket.multifile.store');
	Route::post('/support_ticket/multifile/update', 'SupportTicketController@updateMultifile')->name('support_ticket.multifile.update');
    ...
    Route::post('/workorder/postWorkOrderStatus', 'WorkOrderController@postWorkOrderStatus')->name('workOrder.postWorkOrderStatus');
    Route::post('/workorder/multifile/store', 'WorkOrderController@storeMultifile')->name('workorder.multifile.store');
	Route::post('/workorder/multifile/update', 'WorkOrderController@updateMultifile')->name('workorder.multifile.update');
    Route::post('workorder/getSelectedProducts','WorkOrderController@getSelectedProducts');
	Route::get('workorder/lims_sale_search', 'WorkOrderController@limsSaleSearch')->name('item_requirement.search');
	Route::get('workorder/lims_product_search', 'WorkOrderController@limsProductSearch')->name('item_requirement.search');
    Route::post('workorder/work-data', 'WorkOrderController@workOrderData');
    Route::get('workorder/create', 'WorkOrderController@createWorkOrder')->name('workOrder.create');
    Route::get('workorder/create_workOrder/{id}', 'WorkOrderController@create_workOrder')->name('workOrder.create_workOrder');
    Route::get('workorder/store', 'WorkOrderController@storeWorkOrder')->name('workOrder.store');
    Route::get('workorder/product_work_order/{id}','WorkOrderController@productWOrkOrderData');
    Route::get('workorder/getproduct/{id}', 'WorkOrderController@getProduct')->name('workOrder.getWorkOrderproduct');
    Route::get('workorder/product_workOder/{id}','WorkOrderController@productWorkOrderData');
    Route::get('workorder/{id}/create', 'WorkOrderController@createworkOrderBySale');
	...
	Route::post('quotation_supplier/purchase-data', 'SupplierQuotationController@supplierQuotationData');
	Route::get('quotation_supplier/getcustomergroup/{id}', 'SupplierQuotationController@getCustomerGroup')->name('quotation_supplier.getcustomergroup');

	Route::get('quotation_supplier/lims_sale_search', 'SupplierQuotationController@limsSaleSearch')->name('supplier_quotations.search');
	Route::get('quotation_supplier/lims_product_search', 'SupplierQuotationController@limsProductSearch')->name('product_supplier_quotations.search');

	Route::get('quotation_supplier/getproduct/{id}', 'SupplierQuotationController@getProduct')->name('quotation_supplier.getproduct');
	Route::resource('quotation_supplier', 'SupplierQuotationController');
	...
	// purchase receiving
	Route::resource('purchase_receiving', 'PurchaseReceivingController');
	Route::post('purchases/purchase-receiving-data', 'PurchaseController@purchaseReceivingData');
	Route::get('purchase_receiving/api/v1/purchase_receiving/{id}', 'PurchaseReceivingController@getPurchaseInfo');
	Route::get('purchase_receiving/product_purchase_receiving/{id}','PurchaseReceivingController@productPurchaseReceivingData');
    ...
    Route::post('/expenses/multifile/store', 'ExpenseController@storeMultifile')->name('expenses.multifile.store');
	Route::post('/expenses/multifile/update', 'ExpenseController@updateMultifile')->name('expenses.multifile.update');
	Route::post('expenses/deletebyselection', 'ExpenseController@deleteBySelection');
	Route::resource('expenses', 'ExpenseController');

	Route::resource('documents', 'DocumentController');

	Route::post('/transfers/multifile/store', 'TransferController@storeMultifile')->name('transfers.multifile.store');
	Route::post('/transfers/multifile/update', 'TransferController@updateMultifile')->name('transfers.multifile.update');
	Route::post('transfers/getSelectedProducts','TransferController@getSelectedProducts');
	...
	Route::post('report/profit_loss_location', 'ReportController@profitLossLocation')->name('report.profitLossLocation');
	Route::get('report/all_customer_due', 'ReportController@allCustomerIndex')->name('report.allCustomerIndex');
	Route::get('report/all_supplier_due', 'ReportController@allSupplierIndex')->name('report.allSupplierIndex');
	Route::post('report/product_cost_report', 'ReportController@productCostReport')->name('report.productCostReport');
	// Route::post('report/stock_availability', 'ReportController@stockavailability')->name('report.stockavailability');
	Route::get('report/stock_availability', 'ReportController@stockavailability')->name('report.stock_availability');
    ...
    //holding routes
    Route::get('holding/lims_company_search', 'HoldingController@limsCompanytSearch')->name('holding.search');
    Route::post('holding/getSelectedCompany','HoldingController@getSelectedCompany');
    Route::resource('holding', 'HoldingController');

	//accounting routes
    Route::resource('accounts-categories', 'AccountCategoriesController');
	...
	// Route::post('attendance/attendance-data', 'AttendanceController@attendanceData')->name('attendance.data');

	Route::get('/attendance/checkin', 'AttendanceController@setCheckin')->name('attendance.checkin');
	Route::post('/attendance/checkout', 'AttendanceController@setCheckout')->name('attendance.checkout');
