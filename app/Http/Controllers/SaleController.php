...
class SaleController extends Controller
{
    public function index(Request $request)
    {
        ...
        if($role->hasPermissionTo('sales-index')) {
            ...
            $lims_payment_status_list    = StatusPayment::all();
            $lims_status_list    = Status::all();

            return view('sale.index',compact('starting_date', 'ending_date', 'warehouse_id', 'customer_id', 'lims_customer_list', 'reference_no', 'user_id', 'lims_user_list', 'lims_payment_status_list', 'payment_status', 'sale_status', 'lims_status_list', 'lims_gift_card_list', 'lims_pos_setting_data', 'lims_reward_point_setting_data', 'lims_account_list', 'lims_warehouse_list', 'all_permission'));
        }
        else
    ...
    public function saleData(Request $request)
    {
        ...
        elseif($customer_id != 0)
            $totalData = Sale::where('customer_id', $customer_id)->whereDate('created_at', '>=' ,$request->input('starting_date'))->whereDate('created_at', '<=' ,$request->input('ending_date'))->count();
        elseif($user_id != 0)
            $totalData = Sale::where('user_id', $user_id)->whereDate('created_at', '>=' ,$request->input('starting_date'))->whereDate('created_at', '<=' ,$request->input('ending_date'))->count();
        else
        ...
        if(empty($request->input('search.value'))) {
            ...
            //             ->get();
                ...
                $sales = Sale::with('biller', 'customer', 'warehouse', 'user')
                        ...
                        if($warehouse_id) {
                            $sales = $sales->where('warehouse_id', $warehouse_id);
                        }
                        if($customer_id) {
                            $sales = $sales->where('customer_id', $customer_id);
                        }
                        if($reference_no) {
                            $sales = $sales->where('reference_no', 'LIKE', "%{$reference_no}%");
                        }
                        if($user_id) {
                        ...
                        if($sale_status) {
                            $sales = $sales->where('sale_status', $sale_status);
                        }
                        $sales = $sales->get();
        }
        else
        ...
        if(!empty($sales))
        {
            foreach ($sales as $key=>$sale)
            {
                ...
                elseif($sale->payment_status == 2)
                    $nestedData['payment_status'] = '<div class="badge badge-info">'.trans('file.Partial').'</div>';
                else
                ...
                if(in_array("sales-edit", $request['all_permission'])){
                    if($sale->sale_status != 3)
                        $nestedData['options'] .= '<li>
                            ...
                            <li>
                                <a href="'.route('workOrder.create_workOrder', $sale->id).'" class="btn btn-link"><i class="fa fa-file-word-o"></i> '.trans('file.Create Work Order').'</a>
                            </li>
                            ';
                    else
                        $nestedData['options'] .= '<li>
    ...
    public function create()
    {
        ...
        if($role->hasPermissionTo('sales-add')) {
            ...
            foreach(json_decode(Auth::user()->warehouse_id) as $warehouseId) {
                $lims_warehouse_list[] = Warehouse::where('id', $warehouseId)
            ...
            return view('sale.create',compact('lims_product_list', 'lims_customer_list', 'lims_warehouse_list', 'lims_biller_list', 'lims_pos_setting_data', 'lims_tax_list', 'lims_reward_point_setting_data'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function store(Request $request)
    {
        ...
        if(($lims_reward_point_setting_data->is_active??false) &&  $data['grand_total'] >= $lims_reward_point_setting_data->minimum_amount) {
    ...
    public function getSelectedProducts(Request $request) {
        ...
        foreach($request->ids as $id) {
            $lims_product_data = Product::with('unit', 'tax')->findorfail($id);
    ...
    public function getAllSelectedProducts() {
        $todayDate = date('Y-m-d');
        $data = array();

        $lims_product_all_data = Product::with('unit', 'tax')->get();
        foreach($lims_product_all_data as $lims_product_data) {
            $product[] = $lims_product_data->name;
            if($lims_product_data->is_variant){
                $product[] = $lims_product_data->item_code;
                $lims_product_data->price += $lims_product_data->additional_price;
    ...
    public function importSale(Request $request)
    {
        ...
        if ($document) {
            ...
            $document->move('/document/sale', $documentName);
            $data['document'] = $documentName;
        }
        $item = 0;
    ...
    public function edit($id)
    {
        ...
        if($role->hasPermissionTo('sales-edit')){
            ...
            $lims_product_sale_data = Product_Sale::where('sale_id', $id)->get();
            $lims_product_list = Product::all();
            return view('sale.edit',compact('lims_product_list', 'lims_customer_list', 'lims_warehouse_list', 'lims_biller_list', 'lims_tax_list', 'lims_sale_data','lims_product_sale_data'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
